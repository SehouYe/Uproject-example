  ...
  return (
    <main className="min-h-screen bg-white text-gray-900 p-6">
      <div className="max-w-xl mx-auto">
        ...
        <p className="text-sm text-gray-600 mb-6">Choose native & target languages to get matched.</p>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {LANGS.map(l => (
                <button type="button" key={"n-"+l.code}
                  onClick={()=>toggle(natives,l.code,setNatives)}
                  className={`px-3 py-1 rounded-full border ${
                    natives.includes(l.code) ? "bg-emerald-600 text-white border-emerald-600" : "bg-white"
                  }`}>
                  {l.label}
                </button>
              ))}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {LANGS.map(l => (
                <button type="button" key={"t-"+l.code}
                  onClick={()=>toggle(targets,l.code,setTargets)}
                  className={`px-3 py-1 rounded-full border ${
                    targets.includes(l.code) ? "bg-indigo-600 text-white border-indigo-600" : "bg-white"
                  }`}>
                  {l.label}
                </button>
              ))}
